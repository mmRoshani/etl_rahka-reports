        ...
        self._pipeline = [
            {
                "$match": {
                    "imei": imei,
                    "data.altitude": {"$exist": True},
                    "data.altitude": {"$ne": None},
                    "data.longitude": {"$exist": True},
                    "data.longitude": {"$ne": None},
                }
            },
            {"$project": {"type": 1, "imei": 1, "data": 1, "createdAt": 1}},
            {
                "$addFields": {
            ...
            {
                "$match": {
                    "data_engine": {"$not": {"$in": ["rssi", "connection", "engine"]}},
                    "tag": {"$regex": "^t_"},
                }
            },
            {
